...
/**
 * The intake. The first screen of the report itself.
 *
 * It sat at `/` for a while, on the reasoning that anyone who arrives already
 * knows why they are here. That holds for the person this is built for and for
 * nobody else, and a text box does not explain the argument to a first-time
 * visitor. So `/` explains the sequence and links straight here; the cost is
 * one tap, paid by people who are not yet in a hurry.
 *
 * Three ways in, whichever is fastest for the person: a screenshot, a pasted
 * SMS, or one sentence typed or spoken. The clock still starts on their first
 * interaction here, not on page load and not on the landing page, and the meter
 * is already visible so what is at stake is legible before they have done
 * anything.
 */
...
function Intake() {
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      <div>
        ...
        {image ? (
          <div className="flex items-center justify-between gap-3 rounded-lg border border-line bg-surface px-4 py-3">
            <span className="text-sm font-medium">{copy.intake.imageReady}</span>
            <button
              type="button"
              onClick={() => setImage(null)}
              className="min-h-11 px-2 text-sm text-muted underline underline-offset-2 transition-colors hover:text-text"
            >
              {copy.intake.remove}
            </button>
          </div>
      ...
      {/* The sub copy says one of these is enough. Without a divider the two
          blocks read as two steps, which is the opposite instruction. */}
      <div className="flex items-center gap-3" aria-hidden>
        <span className="h-px flex-1 bg-line" />
        <span className="eyebrow">{copy.intake.or}</span>
        <span className="h-px flex-1 bg-line" />
      </div>
          ...
          <button
            ...
            className={`mt-2 min-h-11 rounded-lg border px-4 text-sm ${
              dictation.listening
                ...
                : "border-line bg-surface text-muted hover:bg-raised-hover hover:text-text"
            }`}
          >
      ...
      <div className="flex flex-col gap-2">
        ...
        <button
          type="button"
          onClick={manual}
          className="min-h-11 text-sm text-muted underline underline-offset-2 transition-colors hover:text-text"
        >
          {copy.intake.skip}
        </button>
      </div>
      ...
      {demo && (
        <div className="card">
          <h2 className="eyebrow">{copy.intake.demo}</h2>
            ...
            {FIXTURES.map((fixture) => (
              <li key={fixture.id}>
                <button
                  ...
                  className="min-h-11 w-full rounded-lg border border-line bg-raised px-3 py-2 text-left transition-colors hover:border-line-strong hover:bg-raised-hover disabled:opacity-40"
                >
