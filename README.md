...
| Extraction eval | `scripts/eval-extract.mjs` | Scores the vision and text paths: accuracy, invention, and what the validator catches |
...
npm run eval:extract  # scores extraction: 8 generated screenshots + 7 text cases, both paths
npm run eval:images   # regenerates the screenshots and their answer key
...
The interrupt had a rigorous eval from the start. Extraction — the thing a user actually
comes here to do — had none, and the image path had never been run even once. That was
backwards, and `npm run eval:extract` fixes it.

It scores three outcomes per field, and only the third is a failure:

- **correct** — matches the answer key, or correctly refused a field that was not there
- **missed** — came back `UNREADABLE` when a value was available. Safe, and by design
- **bad** — a value that is not the true one, which is then either caught by
  `lib/validate.ts` or **escapes into the packet**

Escapes are the only outcome that could send a bank after the wrong account, so they are
what the run fails on. A `missed` never fails it: refusing to read is the safe behaviour,
and failing on misses would create pressure to loosen the validator, which is backwards.

The screenshots are generated by `npm run eval:images`, which writes the answer key from
the same object that draws the image — so a case cannot disagree with its own ground
truth. Fields whose expected answer is `UNREADABLE` are **cut out of the image entirely**
rather than blurred: a number that was never drawn cannot have been read, so a value
returned for one is an invention with no argument available.

Measured 3 September over three passes: **74 of 75 fields correct**, the vision path
**40 of 40** with nothing invented, and **one escape** — on a dictation transcript, the
model rendered "fastcart dot pay at samplebank" as `fastcart@samplebank`. `lib/validate.ts`
did not catch it and cannot: the wrong value is a perfectly well-formed VPA. Use
`--repeat N` to measure stability; escapes across the three passes were 1, 0, 1, and the
run reports the worst pass rather than the kindest.
