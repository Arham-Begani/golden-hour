...

  {
    ...
    claims: [
      ...
      },
      {
        ...
        status: "partial",
        detail:
          "Recorded, and not yet one distribution — and the five runs on record were measured with a stopwatch that started late. Until 8 September the clock began when the chosen image came back from the file picker rather than when the person tapped “Add a screenshot”, so any run that started with a screenshot excludes the time spent finding it. That is fixed and it makes recorded times longer; it also means the existing runs are undercounted by an unknown amount and are not comparable with later ones. Real runs are timed and published in full, slow ones included; demo replays and the automated journey are bucketed separately and excluded on purpose, because they serve a cached extraction and start the clock at the fixture click. Two rules govern the word median and both live in lib/timings.ts so the landing tile and /evidence cannot answer differently: below five recorded runs neither page will use it, and above five neither page will use it while the runs are two groups separated by a gap wider than every other gap put together — as they are now. The middle of a split sample falls in the space between the groups, where no run happened. What is still missing is the reason for the split: runs recorded before 8 September were stored as bare durations with no timestamp and no record of whether the person corrected a field, so the site can show the two groups and cannot account for them. Runs from 8 September onward carry that provenance. The live count is on /evidence rather than typed here — until 6 September this entry read “no unaided human run-throughs have been recorded” while /evidence was displaying two of them, which is what a number hand-copied into a file does.",
      },
      {
